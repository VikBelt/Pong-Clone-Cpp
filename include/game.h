	...
	while (running) {
		...
		while (SDL_PollEvent(&event)) {
			...
			//keys pressed
				...
				if (event.key.keysym.sym == SDLK_ESCAPE || event.key.keysym.sym == SDLK_b) { running = false; }
				//move the paddle up/down upon pressing w and s
				else if (event.key.keysym.sym == SDLK_w || event.key.keysym.sym == SDLK_UP) { keys[paddleOneUp] = true; }
				else if (event.key.keysym.sym == SDLK_s || event.key.keysym.sym == SDLK_DOWN) { keys[paddleOneDown] = true; }
			}
			//keysreleased
			else if (event.type == SDL_KEYUP) {
				if (event.key.keysym.sym == SDLK_w || event.key.keysym.sym == SDLK_UP) { keys[paddleOneUp] = false; }
				else if (event.key.keysym.sym == SDLK_s || event.key.keysym.sym == SDLK_DOWN) { keys[paddleOneDown] = false; }
			}
		}

		// Code to change Paddle One Position
		if (keys[paddleOneUp] == true) { paddle_one.velocity.y = -PADDLE_SPEED; }
		else if (keys[paddleOneDown] == true) { paddle_one.velocity.y = PADDLE_SPEED; }
		else { paddle_one.velocity.y = 0.0; }

		//Update motions
		paddle_one.Update(time);
		GameCpu(game_ball, paddle_two); //game "ai"
		paddle_two.Update(time);
		game_ball.Update(time);

		//check paddle-ball-wall collisions
		CollisionVelocity(game_ball, paddle_one, paddle_two);
		WallEffect(game_ball, one_score, two_score);
